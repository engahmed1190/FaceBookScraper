...
def scroll():
    ...
    while (True):
        try:
            ...
            wait.until(EC.visibility_of_element_located((By.XPATH, "//*[. = 'Loading more results...']")))
...
def login(email, password):
    ...
    try:
        ...
        #options.add_argument('--proxy-server=183.88.41.188:8080')
...
def page_scrape(ids):
	...
	filename = "collected_"+datetime.datetime.now().strftime("%m-%d-%Y-%H-%M-%S")+".txt"
    # execute for all profiles given in input.txt file
	for id in ids:
		...
		for i in range(10000):
			...
			URLs = driver.find_elements_by_xpath('//*[contains(@class, "_2ial")]')
			#div = driver.find_element_by_class_name('_2ial')
			#print(URLs.get_attribute('href'))
			#print (URLs)
			with open(filename, 'a' , encoding='utf-8') as file:
				for item in URLs:
					file.write(str(item.get_attribute('href'))+'\n')
...
def main():
    #ids = ["https://en-gb.facebook.com/" + line.split("/")[-1] for line in open("NBKFans_04-08-2019-01-36-13.txt", newline='\n')]
    ...
    if len(ids) > 0:
        ...
        email = "ososanny@mailing.one"
        password = "Ahmed_2727651"
        print("\nStarting Scraping...")
